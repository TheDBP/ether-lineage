...
# contains, so it has to change the tag too. release.sh proves an artifact by the tag in its
# filename: it picks the preset that omits gapps and oem, then refuses any zip not carrying that
# preset's tag. A clean build carrying reclaimed OEM art under the plain clean tag would walk
# straight through that gate. Deriving the suffix here means the tag cannot disagree with the
# contents, which a hand-written "clean-oem tag=turbo-clean" row could.
forge_preset_tag() {
  local tag own extra
  tag="$(forge_preset_field "$1" tag)" || return 1
  own=" $(forge_preset_field "$1" options | tr ',' ' ') "
  extra=" $(printf '%s' "${EXTRA_OPTIONS:-}" | tr ',' ' ') "
  # Only suffix when oem was added from outside; a preset that declares oem already has its own tag.
  case "$extra" in
    *" oem "*) case "$own" in *" oem "*) ;; *) tag="$tag-oem" ;; esac ;;
  esac
  printf '%s' "$tag"
}
