...
The manifest declares no `uses-library` entries, so the 20.0 module sets no
`LOCAL_OPTIONAL_USES_LIBRARIES`; if an update adds some, `manifest_check.py` fails the build until
they are mirrored there.
...
Re-check `aapt2 dump badging` for `uses-library-not-required` entries (see above) and `unzip -l` for
