...
| bonito-lineage | Pixel 3a XL | 22.2, 23.2 | Working; thin layer over supported upstream. Not public yet. |
| vs995-lineage | LG V20 (Verizon) | 22.2, 23.2 | Builds, flashes and boots. Not public yet. |
...
The `-oem` suffix is derived, not typed. `release.sh` proves an artifact by the tag in its filename,
so an image holding reclaimed assets must never be able to wear the shareable tag.
...
prebuilt/             fetchers for Magisk, F-Droid, Firefox, K-9, KDE Connect, TermOne Plus
