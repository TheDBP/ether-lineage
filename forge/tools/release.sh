...
if [ -n "$ZIP_OVERRIDE" ]; then
  ...
  ZIP="$(ls -t "$OUTDIR"/*"$VT"*.zip 2>/dev/null | head -1 || true)"
fi
[ -n "$ZIP" ] && [ -f "$ZIP" ] || die "no built zip for preset '$VN' (expected $OUTDIR/*${VT}*.zip) -- build it first: PRESET=$VN ./forge/bootstrap.sh"
info "artifact: $(basename "$ZIP") ($(du -h "$ZIP" | cut -f1))"
case "$(basename "$ZIP")" in
  *"$VT"*) ok "filename carries the '$VT' tag" ;;
...
these builds: sideload the new zip, no wipe. The zip does not write recovery.
