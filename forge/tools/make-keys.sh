...
# accident.
#
# Never overwrites. Losing releasekey means every user has to wipe to take the next update, so
# back the directory up somewhere that is not this machine.
set -euo pipefail
...
[ -e "$DIR/releasekey.pk8" ] && die "$DIR already holds releasekey.pk8 -- this never overwrites. Use a new directory."
...
for name in releasekey platform shared media networkstack bluetooth sdk_sandbox testkey verity; do
  gen "$name" 2048
  echo "   $name"
done
...
if [ -n "$AVBTOOL" ]; then
  ...
  # Test and example APEXes are excluded; they are never in an image.
  grep -rhoE 'public_key: *"[^":]+\.avbpubkey"' --include=Android.bp \
