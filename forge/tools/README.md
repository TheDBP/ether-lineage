...
| `lib/presets.sh` | resolves a preset name into an option list and a build tag. `EXTRA_OPTIONS` is unioned in here, and the `-oem` tag suffix is derived here rather than written by hand |
