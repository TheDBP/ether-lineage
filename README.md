...
- **Firefox, F-Droid and K-9 Mail**, each on its own switch.
...
always the `libre` preset: LineageOS plus F-Droid, Firefox and K-9 Mail, no Google apps, not rooted,
none of Nextbit's own artwork.
...
PRESET=libre ./forge/bootstrap.sh    # + F-Droid, Firefox and K-9, still no Google
PRESET=full  ./forge/bootstrap.sh    # + GApps, root, Firefox, F-Droid, K-9
...
`oem` is in no preset. It applies to whichever one you build and appends `-oem` to the tag:
...
upstream stays upstream and what we changed stays legible. One patch per thing it enables, and no
patch undoes another. Each entry below: what broke → what the patch does → what it costs.
